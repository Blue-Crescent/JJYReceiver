...
begin()により受信が成功し、この関数の初回呼び出し時にJJY受信データの変換が行われて時刻が校正されます。(esp32は割り込みハンドラ内でmktime計算ができないため)
...
サンプルスケッチの出力結果は以下の通りです。 受信中(time_tが-1の時)は、lgt8f328pの場合、2136年の時刻を指すようです。esp32の場合は1970年の時刻を指すようです。
